...
def random_key(length):
    ...
    for i in range(length):
        # because no space and - char are allowed in names/alias, so remove the - and space
...
def poolcreateandlist(c,poolnum):
    ...
    for pdlist in pdhddssdlist:
        ...
        if phydrvnum>=5:
            ...
            if poolnum==2:
                poolname = random_key(maxnamelength) + str(phydrvnum)
                tolog(str(phydrvnum) + " phydrvs are in the system, 1 raid 5 or raid 6 level pools will be created and no phydrv is available.")
                raidlevel=random.choice(["5", "6"])
...
def newbuildinstall(c,server):
    ...
    tftpbuildnumber = open("./srvpool/buildnum","r").readline().rstrip()
    # for file in files:
    #     try:
    #         tmp = int(file[-7:-5])
    #     except:
    ...
    if "12." in tftpbuildnumber:

        tftpbuildnumber=tftpbuildnumber[-2:]

        if int(currentbuild.split(".")[-1]) < int(tftpbuildnumber):
    ...
    elif "13." in tftpbuildnumber:
        tftpbuildnumber = tftpbuildnumber[-2:]

        if int(currentbuild.split(".")[-1]) < int(tftpbuildnumber):
            filename = "d5k-multi-13_0_0000_" + str(tftpbuildnumber)
            SendCmd(c, "ptiflash -y -t -s 10.84.2.99 -f " + filename + ".ptif")

            tolog("%s will be updated to the %s" % (filename, server))
    else:
        tolog("Something goes wrong, please check it manually.")
    ...
    versioninfo = SendCmd(c, "about")
    currentbuild = versioninfo.split("Version: ")[1][11:13]
    if currentbuild==tftpbuildnumber:
        tolog("%s is updated to the %s" % (currentbuild, server))
    else:
        tolog("Something goes wrong, please check it manually.")
...
if __name__ == "__main__":
    ...
    if paradict["pooltype"]:
        poolcreateandlist(c, int(paradict["pooltype"]))
    if paradict["volnum"]:
    ...
    if paradict["sparedrvnum"]:
        sparedrvcreate(c, int(paradict["sparedrvnum"]))
